...
export default function Dashboard({ setCurrentView }) {
  return (
    <div className="min-h-screen bg-linear-to-br from-slate-50 via-white to-indigo-50/30 flex flex-col">
      ...
      <header className="sticky top-0 z-50 animate-slideDown backdrop-blur-xl bg-white/80 border-b border-slate-200/50">
        <div className="max-w-7xl mx-auto flex items-center justify-between px-6 py-4">
          <div className="flex items-center space-x-4 hover:scale-105 transition-transform duration-300 cursor-pointer">
            <div className="bg-linear-to-br from-indigo-600 to-indigo-700 p-2 rounded-xl text-white shadow-lg hover:shadow-indigo-500/50 transition-all">
          ...
          <div className="flex items-center space-x-4">
            <div className="flex items-center space-x-2 bg-linear-to-r from-emerald-50 to-blue-50 px-4 py-2 rounded-lg border border-emerald-200/50 text-xs font-semibold text-slate-700 animate-pulse-slow">
            ...
            <button
              ...
              className="flex items-center space-x-2 text-sm font-semibold text-red-600 hover:text-red-700 bg-linear-to-r from-red-50 to-pink-50 hover:from-red-100 hover:to-pink-100 px-4 py-2 rounded-lg transition-all duration-300 border border-red-200/50 hover:border-red-300/50"
            >
      ...
      <main className="max-w-7xl mx-auto p-6 space-y-8 grow w-full">
        {/* Hero Welcome Banner */}
        <div className="bg-linear-to-br from-indigo-600 via-indigo-700 to-slate-900 rounded-2xl p-8 text-white shadow-xl hover:shadow-2xl transition-shadow duration-300 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 overflow-hidden relative animate-slideUp">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 animate-slideUp">
          {[
            ...
            <div 
              key={i} 
              className={`bg-linear-to-br ${stat.bgGradient} border ${stat.borderColor} p-6 rounded-2xl shadow-sm hover:shadow-lg ${stat.hoverBorder} transition-all duration-300 space-y-3 hover:-translate-y-1 animate-slideUp`}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {[
              ...
              <div 
                ...
                className={`p-6 rounded-2xl ${tool.borderColor} border bg-linear-to-br ${tool.bgGradient} ${tool.hoverBg} ${tool.hoverBorder} ${tool.hoverGradient} hover:shadow-lg transition-all duration-300 cursor-pointer space-y-4 hover:-translate-y-1 group animate-slideUp`}
