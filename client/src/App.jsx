...
import { 
  ...
  Zap, AlertTriangle, CheckCircle, Network, Terminal, User, LogOut, ShieldAlert, GitCompare, ShieldCheck
} from 'lucide-react';
...
function App() {
  ...
  const getSeverityBadge = (severity) => {
    switch (severity) {
      ...
      default:
        return <span className="px-2 py-0.5 text-xs font-bold text-slate-700 bg-slate-100 rounded">{severity}</span>;
    }
  };
      ...
      <main className="grow">
        {currentView === 'home' && (
          <div className="space-y-24 pb-24">
            ...
            <section className="relative pt-16 pb-12 px-6 bg-linear-to-b from-white via-slate-50 to-slate-100 border-b border-slate-200">
              ...
              {/* Restored Attractive Hero Image Banner */}
            ...
            {/* Restored Feature Cards with Tech Imagery */}
        ...
        {currentView === 'analyzer' && (
          <div className="max-w-6xl mx-auto p-6 space-y-6">
            ...
            {/* Top Bar with Health Score Badge */}
            <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-2xs flex items-center justify-between">
              <div className="flex items-center space-x-3">
                <div className="p-2 bg-indigo-50 text-indigo-600 rounded-xl"><Code size={20} /></div>
                <div>
                  <h2 className="font-bold text-slate-900 text-sm">Interactive AST Code Analyzer</h2>
                  <p className="text-slate-500 text-xs">Run syntax trees, view diffs, and apply instant refactoring patches.</p>
                </div>
              </div>
              <div className="flex items-center space-x-4">
                {report && (
                  <div className={`px-4 py-1.5 rounded-xl border flex items-center space-x-2 font-bold text-xs ${health.color}`}>
                    <ShieldCheck size={16} />
                    <span>Health Rating: {health.grade}</span>
                  </div>
                )}
                <button
                  onClick={() => setShowDiff(!showDiff)}
                  className="bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold px-3.5 py-2 rounded-xl text-xs flex items-center space-x-1.5 transition"
                >
                  <GitCompare size={14} />
                  <span>{showDiff ? 'Hide Diff Viewer' : 'Side-by-Side Diff'}</span>
                </button>
              </div>
            </div>

            {/* Main Editor & Report Grid */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="flex flex-col bg-white rounded-2xl p-5 shadow-xs border border-slate-200">
                <label className="text-sm font-semibold text-slate-700 mb-2">Source Code Buffer</label>
                <textarea
                ...
                <button
                  ...
                  disabled={loading}
                  className="mt-4 w-full bg-indigo-600 hover:bg-indigo-700 text-white font-semibold py-3 px-4 rounded-xl shadow transition disabled:opacity-50 flex items-center justify-center space-x-2"
                >
                  {loading ? <span>Analyzing AST Trees...</span> : <><span>Run Static Analysis</span><ArrowRight className="w-4 h-4" /></>}
                </button>
              </div>
                ...
                <div className="w-full h-96 bg-slate-50 rounded-xl border border-slate-200 p-4 overflow-y-auto">
                  {!report ? (
                    <div className="h-full flex flex-col items-center justify-center text-slate-400 text-sm text-center px-4">
                      ...
                      <p>Run an analysis to generate inline solutions and patches.</p>
                    </div>
                      ...
                      {report.issuesFound.map((issue, index) => (
                        <div key={index} className="bg-white border border-slate-200 rounded-xl p-4 shadow-2xs text-sm space-y-2">
                          <div className="flex items-center justify-between">
                            <span className="font-bold text-slate-800">Line {issue.line}: {issue.ruleName}</span>
                            {getSeverityBadge(issue.severity)}
                          </div>
                          <p className="text-slate-600 text-xs">{issue.message}</p>
                          
                          <div className="bg-indigo-50/70 border border-indigo-100 rounded-lg p-3 mt-2 space-y-2">
                            <div className="flex items-center justify-between">
                              ...
                              <button
                                onClick={() => handleApplyFix(issue.suggestedFix)}
                                className="text-[11px] bg-indigo-600 hover:bg-indigo-700 text-white font-semibold px-2.5 py-1 rounded shadow-2xs transition"
                              >
                                Apply Fix
                              </button>
                            </div>
            ...
            {/* Side-by-Side Diff Viewer Drawer */}
            {showDiff && (
              <div className="bg-slate-900 text-white rounded-2xl p-6 border border-slate-800 space-y-4">
                <div className="flex items-center justify-between">
                  <h3 className="font-bold text-sm flex items-center space-x-2">
                    ...
                    <span>Side-by-Side Diff Preview (Before vs. Suggested Fix)</span>
                  </h3>
                  <button onClick={() => setShowDiff(false)} className="text-xs text-slate-400 hover:text-white">Close</button>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="bg-slate-950 p-4 rounded-xl border border-slate-800">
                    <span className="text-xs text-red-400 font-mono block mb-2">- Original Buggy Code</span>
                    <pre className="text-xs font-mono text-slate-300 overflow-x-auto">{code || '// No code provided'}</pre>
                  </div>
                  <div className="bg-slate-950 p-4 rounded-xl border border-slate-800">
                    <span className="text-xs text-emerald-400 font-mono block mb-2">+ Refactored Safe Code</span>
                    <pre className="text-xs font-mono text-emerald-300 overflow-x-auto">
                      {report?.issuesFound?.[0]?.suggestedFix || '// Click Apply Fix to preview refactoring'}
                    </pre>
                  </div>
                </div>
              </div>
